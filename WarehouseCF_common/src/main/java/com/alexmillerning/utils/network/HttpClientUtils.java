...
package com.alexmillerning.utils.network;
...
public class HttpClientUtils {
    static final Logger logger = Logger.getLogger(HttpClientUtils.class);
    // 连接主机超时（30s）
    ...
    public static String httpPost(String url, String jsonParam) throws IOException {
        ...
        try {
            ...
            if(logger.isDebugEnabled()){
                logger.debug("服务器[REST] 请求方式[HTTP/POST] 返回数据["+str+"]");
            }
    ...
    public static Map<String, Object> httpPostGetCookies(String url, String jsonParam) throws
        ...
        try {
            ...
            String str = EntityUtils.toString(entity);
            if(logger.isDebugEnabled()){
                logger.debug("服务器[REST] 请求方式[HTTP/POST/COOKIE] 返回数据["+str+"]");
            }
            EntityUtils.consume(entity); // 此句关闭了流
    ...
    public static String httpGet(String url) throws  IOException {
        ...
        try {
            ...
            if(logger.isDebugEnabled()){
                logger.debug("服务器[REST] 请求方式[HTTP/GET] 返回数据["+strResult+"]");
            }
            return strResult;
        } finally {
            response.close();
    ...
    public static String httpGetWithCookies(String url, Header[] headers) throws  IOException {
        ...
        try {
            ...
            String strResult = EntityUtils.toString(entity);
            if(logger.isDebugEnabled()){
                logger.debug("服务器[REST] 请求方式[HTTP/GET/COOKIE] 返回数据["+strResult+"]");
            }
            return strResult;
        } finally {
            response.close();
